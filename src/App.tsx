                ...
                <thead>
                  <tr style={{ background: '#FAFBFC' }}>
                    {RES_HEADERS.map((h, i) => {
                      ...
                      return <th key={i} onClick={() => clickHeader(sk)} style={{ ...headCell, textAlign: RES_ALIGN[i], cursor: sk ? 'pointer' : 'default', color: on ? '#1D9E75' : '#888', userSelect: 'none', padding: '11px 10px', borderBottom: '1px solid #E5E7EB', ...(i === 1 ? { borderRight: '1px solid #E5E7EB' } : {}), ...(i === 7 ? { borderLeft: '1px solid #E5E7EB' } : {}) }}>{h}{on ? (sortDir === 'desc' ? ' ↓' : ' ↑') : ''}</th>
                    })}
                  </tr>
                </thead>
...
function VisitPill({ v }: { v: string }) {
  ...
  return <span style={{
    ...
    ...(isFirst ? { color: '#1D4ED8', background: '#EFF6FF' } : { color: '#9333EA', background: '#FAF5FF' }),
  }}>{visitKo(v)}</span>
}
...
function BookingRow({ b, busy, act, onPropose, onOpen }: { b: Booking; busy: string | null; act: (k: 'confirm' | 'reject', id: string) => void; onPropose: () => void; onOpen: () => void }) {
  ...
  // 사람별 셀들 (이름·생년월일·성별·구분·희망시술) — 동반자 행은 위 구분선만(배경색 없음)
  const personCells = (p: Person, isComp: boolean, lastPerson: boolean) => {
    const pc: React.CSSProperties = { ...td, verticalAlign: 'middle', background: '#fff', borderBottom: lastPerson ? '1px solid #EAEAEA' : '1px solid #F4F4F4' }
    return (
      <>
    ...
    <tbody onClick={onOpen} style={{ cursor: 'pointer' }}>
      {people.map((p, i) => (
        <tr key={p.id}>
          ...
          {i === 0 && shared(b.branchName, { color: '#555', borderRight: '1px solid #E5E7EB' })}
          {personCells(p, i > 0, i === span - 1)}
          {i === 0 && shared(<StatusBadge status={bookerDisplayStatus(b)} />, { borderLeft: '1px solid #E5E7EB' })}
